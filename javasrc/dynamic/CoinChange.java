...
public class CoinChange {
    ...
    private static int nrCombinationsMemo(int[] coins, int n, int sum, int[][] mem) {
        ...
        if (mem[sum][n] < 0) {
            int res = 0;
            res += nrCombinationsMemo(coins, n - 1, sum, mem);
            if (coins[n - 1] <= sum) res += nrCombinationsMemo(coins, n, sum - coins[n - 1], mem);
            mem[sum][n] = res;
        }
        return mem[sum][n];
    }
    ...
    static int nrCombinationsTab(int[] coins, int sum) {
        ...
        for (int i = 1; i <= sum; i++) {
            for (int j = 1; j <= n; j++) {
                mem[i][j] = mem[i][j - 1]; // don't take jth coin
                if (coins[j - 1] <= i) mem[i][j] += mem[i - coins[j - 1]][j]; // take jth coin
            }
        }
        return mem[sum][n];
    }
